...
async function readCacheFile(cacheDir, n) {
  ...
  if (lines.length >= 4) {
    ...
    return fromFileObject(fileObject);
  } else {
...
async function writeCacheFile(cacheDir, previous, fileObject) {
  try {
    const fileObject = toFileObject(previous);
    const content = [
      fileObject.n,
      fileObject.a,
      fileObject.b,
...
async function getBestCacheEntryDatabase(n) {
  ...
  try {
    ...
    const fileObject = fromFileObject(row);
    return fileObject;
  } finally {
    conn.release();
  }
...
async function getFibonacci(inputNString) {
  ...
  let previous = null;
  try {
    console.info('Checking database for best checkpoint...', new Date());
    previous = await getBestCacheEntryDatabase(inputN);
    if (previous != null) {
      console.info('Resuming from checkpoint', previous.n, new Date());
    }
  } catch (err) {
  ...
  while (previous == null || previous.n < inputN) {
    ...
    if (previous.n % 10_000 === 0) {
      ...
      (async () => {
        ...
        await writeCacheFile(cacheDir, taskPrevious, fileObject);
        ...
        console.log("...write to database begins...", taskPrevious.n, new Date());
        await writeCacheDatabase(taskPrevious, fileObject);
        console.log("...written to database", taskPrevious.n, new Date());
      })();
    }
  }
}
